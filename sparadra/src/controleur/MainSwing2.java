...
//    }
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2) {
                    ...
                    if (row != -1) {
                        ...
                        Medecin m = Medecin.getMedecin().get(row);
            ...
            tableMutuelle.addMouseListener(new MouseAdapter() {
                ...
                public void mouseClicked(MouseEvent e) {
                    if (e.getClickCount() == 2) {
                        ...
                        if (row != -1) {
                            Mutuelle m = Mutuelle.getMutuelle().get(row); // adapte selon ta classe Mutuelle
    ...
    private void chargerMutuelle() {
        ...
        for (Mutuelle mutuelle : Mutuelle.getMutuelle()) {
    ...
    private void chargerOrdonnances() {
        ...
        // Cette méthode sera complétée quand la classe Ordonnance sera créée
    ...
    private void modifierMedecin() {
        ...
        try {
            Medecin medecin = Medecin.getMedecin().get(selectedRow);
    ...
    private void modifierMutuelle() {
        ...
        try {
            Mutuelle mutuelle = Mutuelle.getMutuelle().get(selectedRow);
    ...
    private void supprimerMedecin() {
        ...
        int confirm = JOptionPane.showConfirmDialog(this, "Confirmer la suppression ?", "Supprimer", JOptionPane.YES_NO_OPTION);
        if (confirm == JOptionPane.YES_OPTION) {
            Medecin.getMedecin().remove(selectedRow);
            chargerMedecin();
            viderChampsMedecin();
        }
    }
    ...
    private void rechercherMutuelleParNom(String nom) {
        ...
        for (Mutuelle m : Mutuelle.getMutuelle()) {
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            new MainSwing2().setVisible(true);
        });
    }
}